...
class NeuralNetwork():
    ...
    def train(self, X_train, y_train, n_iterations):
        ...
        self.weights_1 = np.random.normal(0, 1, (n_features, self.nodes_hidden))
        self.bias_1 = np.random.normal(0, 1, (1, 1))
        self.weights_2 = np.random.normal(0, 1, (self.nodes_hidden, 1))
        ...
        for i in range(n_iterations):
            ...
            # Forward propagation
            a1 = self.sigmoid(np.dot(self.weights_1.T, X_train) + self.bias_1)
            a2 = self.sigmoid(np.dot(self.weights_2.T, a1) + self.bias_2)
            ...
            dz1 = np.multiply(np.dot(self.weights_2,dz2),np.multiply(a1,1-a1))
            dw1 = (1/n_samples)*np.dot(dz1,X_train.T)
            db1 = (1/n_samples)*np.sum(a1-y_train)

            # Update parameters
            self.weights_2 = self.weights_2 - self.learning_rate*dw2.T
